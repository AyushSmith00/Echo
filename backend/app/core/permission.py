...
def validate_channel_member(
    ...
    membership = db.query(ServerMembers).filter(
        ...
        ServerMembers.server_id == channel.server_id
    ).first()
...
def validate_server_admin(db: Session, server_id: int, user_id: int):
    membership = db.query(ServerMembers).filter(
        ServerMembers.user_id == user_id,
...
def validate_server_member(db: Session, server_id: int, user_id: int):
    membership = db.query(ServerMembers).filter(
        ServerMembers.user_id == user_id,
        ServerMembers.server_id == server_id
    ).first()
